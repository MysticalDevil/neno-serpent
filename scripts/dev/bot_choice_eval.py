#!/usr/bin/env python3
"""Evaluate choice-decision dataset quality."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate choice dataset accuracy/top-k.")
    ...
    with Path(args.dataset).open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            oracle_type = safe_int(row.get("oracle_type", "-1"))
            if rank <= 0:
                continue
            total += 1
            rank_sum += rank
            ...
            if rank <= 2:
                top2 += 1
            bucket = by_type.setdefault(oracle_type, {"samples": 0.0, "top1": 0.0, "top2": 0.0})
            bucket["samples"] += 1.0
            if rank <= 1:
                bucket["top1"] += 1.0
            if rank <= 2:
    ...
    for buff_type, bucket in sorted(by_type.items(), key=lambda item: item[0]):
        samples = int(bucket["samples"])
        by_type_report[str(buff_type)] = {
            "samples": samples,
    ...
    print(
        f"[bot-choice-eval] samples={total} top1_acc={top1_acc:.4f} top2_acc={top2_acc:.4f} "
        f"avg_rank={avg_rank:.4f}"
    )
    print(f"[bot-choice-eval] report={report_path}")
    return 0
